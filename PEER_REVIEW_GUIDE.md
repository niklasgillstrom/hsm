...
- Production code. The `SignatoryRightsVerifier` default is `fail-closed` (intentional). For the state of the Azure/Google attribute handling and the BankID trust anchors, see "Corrections after documentation-versus-code review" below.
...
- **`BankIdSignatureVerificationTest` (9 tests)** covers the BankID path end to end against material generated by `BankIdFixture`: a throwaway root, bank CA, personal certificate and OCSP responder, an XML-DSig-signed response and a signed OCSP response whose nonce binds to it. One test is the accepted case; the other eight are the rejections that matter — revoked certificate, OCSP response signed with the wrong key, responder issued by a different CA, a response legitimately issued for one signature presented with another, a response with no nonce, payload injected outside the element the Reference covers, a duplicated `bankIdSignedData`, and a chain not rooted at the pinned anchor.
